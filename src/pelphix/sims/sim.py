...
class PelphixSim(PelphixBase, Process):
    ...
    MIN_ANGLE_BOUND = math.radians(2)  # smaller than this is just random
    MAX_TIP_BOUND = 10
    MIN_TIP_BOUND = 2
    ...
    def sample_wire_advancement(
        ...
        if progress < 0.1:
            new_progress = np.random.uniform(0.08, 1.5)
        else:
    ...
    def sample_procedure(self, procedure_idx: int) -> bool:
        ...
        while not (state := state.next()).is_finished() and frame_id < self.max_procedure_length:
            ...
            if state.activity == Activity.start:
                ...
                # We only want to insert if we know that we're going to take an image.
                # THis is a bit hacky.
            ...
            if state.acquisition == Acquisition.start:
                ...
                if state.activity == Activity.insert_wire:
            ...
            if state.frame == Frame.start:
                ...
                if (
                    state.task.is_wire()
                    and not wire_placed[state.task]
                    and state.previous is not None
                    and state.previous.frame == Frame.assessment
                ):
            ...
            if state.activity == Activity.position_wire:
                ...
                if (
                    ...
                    and screw_placed[state.task.get_wire()]
                ):
                    ...
                    # The wire doesn't look good. Unknown what to do here.
                    log.warning(f"Wire doesn't look good. Unknown what to do here. Continuing...")
            else:
