...
class Tool(deepdrr.Volume, ABC):
    ...
    Place files in the following directories, relative to the repo root:

    .
    └── data
        └── tools
            ├── {snakecase(ToolClassName)}
            │   ├── material_1.stl
            │   │── material_2.stl
            │   └── material_3.stl
            └── {snakecase(ToolClassName2)}
                ├── material_1.stl
                │── material_2.stl
                └── material_3.stl
    ...
    def trajectory_in_world(self) -> geo.Ray3D:
        return geo.Ray3D.from_pn(self.tip_in_world, self.tip_in_world - self.base_in_world)
