...
from .workflow_state import Task, Activity, Acquisition, Frame, WorkflowState, FrameState
...
class PelphixSim(PerphixBase, Process):
    ...
    def evaluate_view(self, state: WorkflowState, device: SimpleDevice, view: geo.Ray3D) -> float:
        """Evaluate a view.
    ...
    def sample_procedure(self, procedure_idx: int) -> bool:
        ...
        state = WorkflowState(max_corridors=max_corridors)
    ...
    def generate(self):
        ...
        if self.num_workers == 0:
            ...
            # Start workers
            workers: list[PelvicWorkflowsSimulation] = []
            for i in range(self.num_workers):
                os.environ["CUDA_VISIBLE_DEVICES"] = str(i % torch.cuda.device_count())
                worker = PelvicWorkflowsSimulation(
